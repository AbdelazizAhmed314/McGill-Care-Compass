    ...
    *,
    ...
    try:
        with closing(sqlite3.connect(temporary_db)) as connection:
            ...
            pd.DataFrame([signature.to_dict()]).to_sql(
                "corpus_signature", connection, if_exists="replace", index=False
            )
        with closing(
            sqlite3.connect(f"file:{temporary_db.resolve()}?mode=ro", uri=True)
        ) as connection:
            actual_counts = {
        ...
        with closing(
            ...
            stored_signature = dict(
                zip(
                    [item[1] for item in connection.execute("PRAGMA table_info(corpus_signature)")],
                    connection.execute("SELECT * FROM corpus_signature").fetchone(),
                    strict=True,
                )
            )
        if {key: str(value) for key, value in stored_signature.items()} != {
            key: str(value) for key, value in signature.to_dict().items()
        }:
            raise RuntimeError("New SQLite metadata failed corpus-signature validation.")
