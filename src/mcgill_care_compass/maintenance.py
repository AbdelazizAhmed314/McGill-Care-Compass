...
def build_maintenance_report(
    ...
    *,
    ...
    needs_attention = bool(
        ...
        or freshness["new_count"]
        or freshness["fetch_failed_count"]
        or freshness["stale_count"]
        or failed_sources["count"]
        or any(section["rows_with_missing_data"] for section in missing_data.values())
        or coverage["categories_without_chunks"]
        or any(item["count"] for item in quality.values())
    )
    return {
...
def format_maintenance_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Missing optional source-updated date: "
        f"{freshness['source_updated_at_missing_count']}",
        "",
...
def _failed_source_report(
    pages: list[dict[str, str]], *, example_limit: int
) -> dict[str, Any]:
    failed = []
    for row in pages:
        status = _integer(row.get("http_status", ""))
        if row.get("fetch_error") or row.get("drift_status") == "fetch_failed" or (
            status is not None and not 200 <= status < 400
        ):
            failed.append(
                {
                    "canonical_url": row.get("canonical_url", ""),
...
def _skipped_link_report(
    links: list[dict[str, str]], *, example_limit: int
) -> dict[str, Any]:
    skipped = [
        row
        for row in links
...
def _coverage_report(
    pages: list[dict[str, str]], chunks: list[dict[str, str]]
) -> dict[str, Any]:
    ...
    by_category = {
        category_id: {
            ...
            "mode": (
                "guardrail_only"
                if category_id in GUARDRAIL_ONLY_CATEGORIES
                else "retrieval"
            ),
        }
