...
GUARDRAIL_CLASSES = {"attack", "benign", "safety", "fallback"}
RESPONSE_STATUSES = {
    "matched",
    "emergency",
    "unsafe_input",
...
def load_scenario_set(path: Path = DEFAULT_SCENARIOS) -> dict[str, Any]:
    ...
    for scenario in payload["scenarios"]:
        ...
        if expected_status not in RESPONSE_STATUSES:
            raise ValueError(
                f"Unsupported expected_status in {scenario_id}: {expected_status}"
            )
        ...
        if scenario.get("must_include_source_link") is not True:
            raise ValueError(
                f"Scenario {scenario_id} must require an official source-link check."
            )
        if limitation_notice(category_id) and scenario.get("must_include_limitation") is not True:
            raise ValueError(
                f"Scenario {scenario_id} must require the governed limitation check."
            )
        ...
        if kind == "relevance":
            ...
            if not isinstance(categories, list) or not categories:
                raise ValueError(
                    f"Relevance scenario {scenario_id} needs expected_categories."
                )
            ...
            if guardrail_class == "attack" and (
                ...
                raise ValueError(
                    f"Attack scenario {scenario_id} needs expected_guardrail_reasons."
                )
            ...
            has_redacted_emergency = has_redacted_emergency or bool(
                scenario.get("must_redact_query")
                and scenario.get("expected_status") == "emergency"
            )
    ...
    if missing_classes:
        raise ValueError(
            "Scenario set is missing guardrail classes: "
            + ", ".join(sorted(missing_classes))
        )
    missing_attacks = REQUIRED_ATTACK_REASONS - attack_reasons
    if missing_attacks:
        raise ValueError(
            "Scenario set is missing attack classes: "
            + ", ".join(sorted(missing_attacks))
        )
...
def run_evaluation(
    ...
    *,
    ...
    results = [
        evaluate_scenario(scenario, retriever=retriever)
    ...
    return {
        ...
        "summary": {
            ...
            "attack_detection": round(
                sum(result.passed for result in attacks) / len(attacks), 4
            ) if attacks else 0.0,
            "benign_scenarios": len(benign),
            "benign_scenarios_passed": sum(result.passed for result in benign),
            "benign_pass_through": round(
                sum(result.passed for result in benign) / len(benign), 4
            ) if benign else 0.0,
...
def _guardrail_checks(
    ...
    if scenario.get("must_include_limitation"):
        ...
        checks["limitation"] = bool(
            response.limitation_notice
            and (not required or required in response.limitation_notice)
        )
